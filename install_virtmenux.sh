...
# ==========================================================
# virtmenux- A menu-driven script for Compute Node Mgmt.
# ==========================================================
# Author      : Mayur ~ Ganesh
# Copyright   : (c) 2025 
...
# ==========================================================
# Description:
# This script installs and configures virtmenux, a menu-driven
# tool for managing Virtulisation Compute Node.
...
# - Creates necessary directories for storing virtmenux data.
# - Downloads required files , including:
# - #Cache file (`cache.json`) for translation caching.
# - Utility script (`utils.sh`) for core functions.
# - Main script (`menu.sh`) to launch virtmenux.
# - Sets correct permissions for execution.
# - Displays final instructions on how to start virtmenux.
#
# This installer ensures a smooth setup process and prepares
# the system for running virtmenux efficiently.
# ==========================================================
...
uninstall_mithubmenu() {
    local install_type="$1"
    local force_clean="$2"
    
    if [ "$force_clean" != "force" ]; then
        if ! whiptail --title "Uninstall MITHUBMenu" --yesno "Are you sure you want to uninstall MITHUBMenu?" 10 60; then
            return 1
        fi
    fi
    
    echo "Uninstalling MITHUBMenu..."
    ...
    if [ -f /etc/motd.bak ]; then
        ...
        sed -i '/This system is optimised by: virtmenux/d' /etc/motd
    fi
    
    echo "MITHUBMenu has been uninstalled."
    return 0
}
    ...
    case "$current_type-$new_type" in
        "translation-1"|"translation-normal")
            if whiptail --title "Installation Type Change" \
                ...
                uninstall_mithubmenu "translation" "force" >/dev/null 2>&1
                return 0
            else
                return 1
            fi
...
show_progress() {
    ...
    echo -e "\n${BOLD}${BL}${TAB}Installing MITHUBMenu: Step $step of $total${CL}"
    echo
    msg_info2 "$message"
}
...
show_installation_options() {
    ...
    local menu_title="virtmenux Installation"
    local menu_text="Choose installation type:"
    
    if [ "$current_install_type" != "none" ]; then
        case "$current_install_type" in
            "translation")
                menu_title="virtmenux Update - Translation Version Detected"
                ;;
            "normal")
                menu_title="virtmenux Update - Normal Version Detected"
                ;;
            "unknown")
                menu_title="virtmenux Update - Existing Installation Detected"
                ;;
        esac
    fi
    
    INSTALL_TYPE=$(whiptail --backtitle "virtmenux" --title "$menu_title" --menu "\n$menu_text" 14 70 2 \
...
install_mithubmenu() {
    ...
    case "$INSTALL_TYPE" in
        "1")
            show_mithubmenux_logo
            msg_title "Installing virtmenux - Normal Version"
            install_normal_version
            ;;
        "2")
            show_mithubmenux_logo
            msg_title "Installing virtmenux - Translation Version"
            install_translation_version
            ;;
        *)
    ...
    msg_title "$(translate "virtmenux has been installed successfully")"
    echo -ne "${GN}"
    type_text "$(translate "To run virtmenux, simply execute this command in the console or terminal:")"
    echo -e "${YWB}    menu${CL}"
    echo
}
